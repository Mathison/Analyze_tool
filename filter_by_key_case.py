...
def read_folder(path,date_name,platform):
    ...
    for filename in sorted(os.listdir(path)):
        ...
        k_data = read_json(path+filename,platform)
        for key in k_data:
            #print(key)
            if key not in total_data:
                total_data[key] = []
            total_data[key] += k_data[key]
    '''
    return total_data
...
def check_key_with_output(text,keywords):
    text = text.replace(':',' ')
    text = text.replace('/',' ')
    text = text.replace('#',' ')
    text = text.replace('@',' ')
...
if __name__ == '__main__':
    ...
    '''
    dict_file = open('key_dict_fake_4.txt','w')
    for i in sorted(key_dict):
        print((i,key_dict[i]),end = '\n')
        dict_file.write(str(i) + ' ' + str(key_dict[i]) + '\n')
    dict_file.close()
    '''
